...
@section('content')
    ...
    <div class="wrapper position-relative overflow-hidden">
        ...
        ?>
        {{ date('H:i:s') }}
        {{ \Carbon\Carbon::createFromFormat('d F, Y', date('d F, Y', strtotime(date('H:i:s'))))->locale('es')->isoFormat('D MMMM, YYYY') }}
                    ...
                    @foreach ($data['preguntas'] as $key => $item)
                        ...
                        <tr>
                            ...
                            <td> Agrado<input for="opt_1" type="radio" class="required"
                                    name="preguntas[{{ $item->id_area }}][{{ $item->id_pregunta }}]" value="3" checked>
                            </td>
                            <td>

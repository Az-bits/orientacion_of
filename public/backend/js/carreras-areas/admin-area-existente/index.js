...
$(document).ready(function () {
    ...
    btnState = {
        id: null,
        add: function () {
            modalTitle.text("nuevo area UPEA");
        ...
        edit: function (id) {
            modalTitle.text("editar area UPEA");
            utilities.resetForm(form);
            this.id = id;
            btnSubmit.prop("disabled", false);
        },
    ...
    table = $("#datatable").DataTable({
        ...
        ajax: {
            url: "/admin/area-existente",
        },
        columns: [
            { data: "id_area_existente" },
            { data: "nombre" },
            {
                data: null,
                targets: -1,
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    // console.log(row);
                    return _ACTIONS("area UPEA", row.id_area_existente);
                },
            },
        ],
    ...
    table.on("click", ".delete", function () {
        let id = $(this).data("id");
        az.showSwal("warning-message-delete", `/admin/area-existente/${id}`);
    });
    ...
    btnSubmit.on("click", function () {
        ...
        if (!id) {
            saveRegister("/admin/area-existente", "POST");
        } else {
            saveRegister(`/admin/area-existente/${id}`, "PUT");
        }
    });
    function edit(id, table) {
        let reg = utilities.getByID(id, table, "id_area_existente");
        modalEl.modal("show");
